...
usage() {
	...
	echo >&2 "  -z # execute a post-deboostrap script (post-deboostrap.sh)"
...
while getopts v:i:a:p:dstz name; do
	case "$name" in
		p)
			http_proxy="$OPTARG"
			;;
...
if [ "$postDeboostrapScript" ]; then
	# execute the post-deboostrap.sh script to install additionnals packages
	if [ -f "$returnTo"/files/post-deboostrap.sh ]; then
		sudo mount --bind /dev dev
		sudo mount -t proc /prox proc
		sudo mount -t sysfs /sys sys
		sudo cp -pr "$returnTo"/files tmp/
		sudo chmod +x -R tmp/files/*
		sudo chroot . /tmp/files/post-deboostrap.sh
		sudo umount dev
		sudo umount proc
		sudo umount sys
	fi
